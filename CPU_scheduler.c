...
typedef struct myProcess {
    ...
    int arrivalTime;
    int CPUburst;
    int IOburst; //IO 작업을 하는 시간
    int IOrequest; //IO 요청 횟수
    int IOrequestremain; //남은 IO 요청 횟수
    int CPUremainingTime;
    int IOremainingTime; //남은 IO 작업 시간
    int IOwait; //running process의 IO 작업까지의 남은 시간
    int waitingTime;
    int turnaroundTime;
...
evalPointer evals[30]; //save evaluations
int cur_eval_num = 0;
...
void sort_JQ() { //JobQueue pid order
    int i, j;
...
//JobQueue에서 해당 pid를 가지고 있는 process의 index를 리턴한다.
...
//process 하나를 JobQueue에서 제거하고 빈 공간을 수축을 통해 없앤다.
...
// JobQueue를 cloneJQ에 저장 후 clone에서 복사한 JobQueue로 알고리즘 테스트트
void clone_JQ() {
...
//readyQueue에서 해당 pid를 가지고 있는 process의 index를 반환환.
...
//process 하나를 readyQueue에서 제거하고 빈 공간을 수축을 통해 없앤다.
...
//waitingQueue에서 해당 pid를 가지고 있는 process의 index를 리턴.
...
//process를 waitingQueue의 마지막에 넣음음
...
//process 하나를 waitingQueue에서 제거하고 빈 공간을 수축을 통해 없앤다.
...
//process 하나를 만들어 JobQueue에 넣음
processPointer createProcess(int pid, int priority, int arrivalTime, int CPUburst, int IOburst, int IORequest, int IOwait) { 
...
processPointer FCFS_alg() {
        ...
        processPointer earliestProc = readyQueue[0]; //ready Queue에 가장 먼저 도착한 process를 찾는다.
        ...
        if (earliestProc != NULL){
            ...
            if(runningProcess != NULL) { //이미 수행중인 프로세스가 있었다면 preemptive가 아니므로 기다린다.  
	            ...
	            */	
				...
				return removeFrom_RQ(earliestProc); //가장 먼저 도착한 process를 ReadyQueue에서 꺼내서 running process로 선택택
			}
...
processPointer SJF_alg(int preemptive) {
	...
	if(shortestJob != NULL) { //readyQueue가 비어있지 않은 상황황
		int i;
        for(i = 0; i < cur_proc_num_RQ; i++) { //readyQueue의 모든 process를 확인하여 남은 시간이 가장 짧은 process를 찾는 과정정
		...
		if(runningProcess != NULL) { //이미 수행중인 프로세스가 있을 때 
					...
					if(runningProcess->CPUremainingTime >= shortestJob->CPUremainingTime) {
						if(runningProcess->CPUremainingTime == shortestJob->CPUremainingTime) { //남은 시간이 같을 경우먼저 도착한 process가 먼저 수행된다.
		                    if (runningProcess->arrivalTime < shortestJob->arrivalTime){
								...
								return runningProcess; //arrivalTime까지 같으면 굳이 preempt하지 않는다. 
						}
				...
				return removeFrom_RQ(shortestJob); //수행중인 process가 없을 시 CPU burst remain이 가장 짧은 process 선택택
			}
...
processPointer PRIORITY_alg(int preemptive) {
	...
	if(importantJob != NULL) { //readyQueue가 비어있지 않은 경우
		int i;
        for(i = 0; i < cur_proc_num_RQ; i++) { //priority가 높은 process를 찾는 과정 (숫자가 낮을수록 priority 높음음)
	...
	processPointer earliestProc = readyQueue[0]; //가장 먼저 도착한 process를 찾는다.
        ...
        if (earliestProc != NULL){
            ...
            if(runningProcess != NULL) { //이미 수행중인 프로세스가 있었다면
				...
				if(timeConsumed >= time_quantum){ //process가 실행된 시간을 확인하여 time quantum 이상이면 readyQueue로 보냄냄 		
					insertInto_RQ(runningProcess);
...
void simulate(int amount, int alg, int preemptive, int time_quantum) { //amount 시점이 흐른 뒤의 상태 -> 반복문에 넣어서 사용 
    ...
    //running process가 바뀐 것을 감지하여 gantt chart에 기록
	printf("%d: ",amount);
    if (tempprevpid != temppid) {
        ...
        //pid 기록, running process가 없을 경우 idle 기록
        if (runningProcess == NULL) { 
    ...
    for(i = 0; i < cur_proc_num_WQ; i++) { //waitingQueue에 있는 process들이 IO 작업을 수행한다. 
		if(waitingQueue[i]) {
			...
			if(waitingQueue[i]->IOremainingTime <= 0 ) { //IO 작업이 완료된 경우 
                ...
                if (waitingQueue[i]->IOrequestremain > 0) waitingQueue[i]->IOremainingTime = waitingQueue[i]->IOburst; //IO remaining time 초기화
    ...
    if (runningProcess != NULL) { //running 중인 프로세스가 있다면 실행시킴
        ...
        if (runningProcess->CPUremainingTime <= 0) { //모두 수행이 된 상태라면, terminated로 보내준다. 
            ...
            if (runningProcess->IOremainingTime > 0) {  
                if (runningProcess->IOwait <= 0) { //IO 작업을 수행해야 한다면, waiting queue로 보내준다.
                    runningProcess->IOwait = rand() % 5 + 1; //다음 IO 대기시간 재부여
...
void createProcesses() {
    ...
    case 1:
        ...
        printf("How many I/O processes do you want to simulate? (0~%d)\n", totalProcessNum);
    ...
    case 2:
        ...
        for (i = 0; i < totalProcessNum; i++) {
            ...
            createProcess(i + 1, tpriority, tarrivaltime, tCPUburst, tIOburst, tIOrequest, rand() % 5 + 1); //IO wait은 random
        }
...
void main() {
    ...
    while (1) {
        ...
        puts("0. See Job Queue / 1. FCFS / 2. SJF / 3. PRIORITY / 4. RR / 5. Evaluate algorithms / 6. Exit");
        ...
        // 빈 입력 체크
        if (input[0] == '\n') {
            puts("<ERROR>Empty input! Please enter a number from 0 to 6.");
            continue;
        }

        if (sscanf(input, "%d", &algorithm) != 1 || algorithm < 0 || algorithm > 6) {
            puts("<ERROR>Wrong input! Please enter a number from 0 to 6.");
            continue;
        }
        ...
        switch (algorithm) {
            ...
            case 5: //모든 algorithm 한번씩 실행 후 분석 결과 표시시
                clear_evals();
